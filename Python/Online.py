...
for lett in alfabeto:
    ...
    out_trajectories = ppp + "strect_online_trajectories_/"+lett+".txt"
    out_error = ppp + "stretch_online_dis_ERROR/error_"+lett+".txt"
    out_jon_variables_on = ppp + "join_variables_online/"+lett+".txt"
    ...
    for i in coordinates:
        if lett == 'i':
            target_point = [x_off_ste+i[2], off_set - (i[0]/100), i[1]]
        else:
        ...
        if k == 0:
            verification.append(chain.forward_kinematics(q_soln)[:3, 3])
        ...
        q_verification = get_current_configuration()
        verification.append(chain.forward_kinematics(q_verification)[:3,3])
        out__.append(np.linalg.norm(chain.forward_kinematics(q_verification)[:3, 3] - target_point))
        tempo.append(time.time())
            
    with open(out_trajectories, 'w') as file:

        ########################################## ONLINE
        for riga in verification:
    ...
    with open(out_jon_variables_on, 'w') as file:
    ...
    with open("online/tempo_esecuzione/"+lett+".txt", "w") as file:
        for riga in tempo:
            # Scrivi la riga nel file di testo
            file.write(str(riga) + '\n')
